...
from .zx_env_circuit import ZXCalculus
...
class VecZXCalculus():
    ...
    def step(self, actions:np.ndarray) -> tuple:
        ...
        returns: tuple of lists of observations, rewards and dones
        """
        
  
        results = np.array([env.step(act) for env, act in zip(self.env_list, actions)], dtype="O")
        obs = list(results[:, 0])
        rewards = np.array(results[:, 1], dtype=np.float32)
        done = np.array(results[:, 2], dtype=np.int32)
    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                pipe.send(((observation, reward, done), True))
            elif command == "close":
                pipe.send((None, True))
                break
            else:
